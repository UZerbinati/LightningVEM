function [K_local, f_local, polygon] = vem_diffusion_reaction_element(domainMesh, matProps, polynomial, index, f)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FUNCTION: vem_diffusion_convection_element
%
% Created by : M. Trezzi
%
%---------------------------------------------------------------------------------------------------
...
% ==========================
% May  29, 2022: changed the basis function
...
base_val_int   = base_evalutation_interior(p_quad, polynomial, polygon);
grad_val_int   = grad_evalutation_interior(base_val_int, p_quad, polynomial, polygon);

base_val_bound = base_evalutation_boundary(base, b_quad, polynomial, polygon);
grad_val_bound = grad_evalutation_boundary(base_val_bound, b_quad, polynomial, polygon);
...
Pi0_val_int = Pi_evaluate(Pi0, base_val_int);                                                        %Evalutation of the projection 
...
K_local = matProps.epsilon .* Pistar' * Gtilde * Pistar;

K_local = (matProps.epsilon + matProps.sigma.* polygon.diameter.^2) .* Iminus' * Iminus + K_local;

%% CONSTRUCT THE MASS MATRIX
K_local = K_local + matProps.sigma .* Pi0' * H * Pi0;
