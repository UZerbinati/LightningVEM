function [u, ux, uy, maxerr, f, Z, Zplot, A] = laplace(P, varargin) 
...
%% Set up the problem
[g, w, ww, pt, dw, tol, beta, steps, plots, ...        % parse inputs
...
for stepno = 1:maxstepno
   ...
   df = @(z) reshape(gzeval(z(:),wc,...               % vector and matrix inputs
              cc,H,pol,d,arnoldi,scl,n),size(z));    % to u and f both allowed
   u  = @(z) real(f(z));
   ux = @(z) imag (1i*df(z));
   uy = @(z) real(1i*df(z));
...
function [fZ] = gzeval(Z,wc,cc,H,pol,d,arnoldi,scl,n) 
ZZ = [wc; Z];
if arnoldi
   Q  = ones(size(ZZ));
   Qd = zeros(size(ZZ));
   ...
   Qd  = (1:n).*((ZZ-wc)/scl).^(0:n-1)/scl;
   Qd  = [zeros(size(ZZ)) Q];
end
if length(pol) > 0
   fZZ = [Qd -d./((ZZ-pol).^2)]*cc;
else
   fZZ = Qd*cc; 
end
fZ = fZZ(2:end); 
...
function [g, w, ww, pt, dw, tol, beta, steps, plots, slow, ...
...
while j < nargin
   ...
   elseif strcmp(v,'beta'), j = j+1; beta = varargin{j-1};
